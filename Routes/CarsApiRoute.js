...
router.get('/', async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: 'Error occurred while getting car data' });
    }
});
...
router.get('/make_id', async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: 'Error occurred while fetching distinct make_id' });
    }
});
...
router.get('/make_id/:makeId/year-name', async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ message: 'Error occurred while fetching year and name' });
    }
});
...
router.get('/make_id/:makeId/year/:year/name/:name/trim', async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ message: 'Error occurred while fetching trim' });
    }
});
